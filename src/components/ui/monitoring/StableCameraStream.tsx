...
>(({ metadata }, ref) => {
  ...
  useEffect(() => {
    console.log('🎥 [CAMERA METADATA DEBUG] Received metadata:', metadata);
    console.log('🎥 [CAMERA METADATA DEBUG] ultrasonic:', metadata?.ultrasonic);
    console.log(
      '🎥 [CAMERA METADATA DEBUG] distanceTraveled:',
      metadata?.distanceTraveled,
    );
  }, [metadata]);
  ...
  useEffect(() => {
    console.log('🔍 [CAMERA DEBUG] Config changed:', {
      config,
      cameraUrl: config?.cameraStreamUrl,
      hasConfig: !!config,
    });
  ...
  const cameraUrl = useMemo(() => {
    ...
    // Add debug logging
    console.log('🎥 [CAMERA] Config object:', config);
    console.log('🎥 [CAMERA] Camera URL from config:', url);
    console.log('🎥 [CAMERA] URL is empty:', !url || url.trim() === '');
  ...
  const sendQualityToESP32 = async (quality: 'fast' | 'normal' | 'slow') => {
    try {
      ...
      if (ip) {
        ...
        console.log(
          `🎥 [QUALITY] Sending quality setting to ESP32: ${qualityUrl}`,
        );
        // Send HTTP request to ESP32 to change quality
        await fetch(qualityUrl, {
          method: 'GET',
        }).catch((err) => {
          console.warn('⚠️ [QUALITY] Failed to send quality setting:', err);
        });
      }
    } catch (error) {
      console.error('❌ [QUALITY] Error sending quality setting:', error);
    }
  };
...
Camera URL: ${cameraUrl}`);
  ...
  if (!cameraUrl || !isCameraConfigured) {
    console.log('🚫 [CAMERA DEBUG] No camera configured:', {
      cameraUrl,
      isCameraConfigured,
      config,
      configExists: !!config,
      configCameraUrl: config?.cameraStreamUrl,
    });
    return (
        ...
        <div className='text-center text-gray-400 max-w-md mx-auto p-6'>
          ...
          {process.env.NODE_ENV === 'development' && (
            <div className='mt-4 p-3 bg-gray-800 rounded-lg text-xs text-left'>
              ...
              <p className='text-gray-500'>Configured: {isCameraConfigured ? 'yes' : 'no'}</p>
              <p className='text-gray-500'>Config: {config ? 'loaded' : 'null'}</p>
              <p className='text-gray-500'>Config URL: {config?.cameraStreamUrl || 'empty'}</p>
            </div>
          )}        </div>
      </div>
    );
  ...
  return (
    <div className='relative w-full h-full bg-black rounded-2xl overflow-hidden'>
      ...
      </div>{' '}      {/* HUD Overlays - show when connected (with or without metadata) */}
      {isConnected && (
        <>
      ...
      </div>{/* Status Indicator - Always visible */}
      <div className='absolute bottom-4 left-4 flex items-center gap-2 bg-black/70 px-3 py-2 rounded-lg z-30 border border-white/20'>
        <div className={`w-2 h-2 rounded-full ${isConnected && !error ? 'bg-green-500 animate-pulse' : 'bg-red-500'}`}></div>
      ...
      </div>      {/* Compact Obstacle Warning - Top Left */}
      ...
      <div className='absolute bottom-4 right-4 z-30'>
        <div className='bg-gradient-to-br from-gray-900/95 to-black/95 backdrop-blur-sm rounded-xl p-3 border border-white/10 shadow-xl'>
          {(() => {
            ...
            return (              <div className={`relative bg-gradient-to-r ${status.bgGradient} border ${status.borderColor} rounded-lg p-2 ${status.pulse ? 'animate-pulse' : ''}`}>
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <div className='w-px h-8 bg-white/20'></div>                  {/* Right Side: Compact Progress Bar and Stats */}
                  <div className='flex flex-col gap-1.5 flex-1'>
                    ...
                    <div className='relative w-full h-1.5 bg-black/30 rounded-full overflow-hidden'>
                      <div 
                        className={`h-full transition-all duration-500 ease-out ${
                          distance <= 20 ? 'bg-red-500' : 
                          distance <= 50 ? 'bg-yellow-500' : 
                          distance <= 100 ? 'bg-green-500' : 'bg-gray-500'
                        }`}
                        style={{
                          width: distance > 0 ? `${Math.min((distance / 100) * 100, 100)}%` : '0%'
                        }}
                      />
                      {/* Danger zone markers */}
                    ...
                    <div className='flex justify-between text-xs'>
                      <div className='flex items-center gap-1'>
                        <Icon icon='fluent:location-24-regular' width={8} height={8} className='text-blue-400' />
                        <span className='text-gray-400'>
                          {(distanceTraveled / 100).toFixed(1)}m
                        </span>
                      </div>
                      <div className='flex items-center gap-1'>
                        <Icon icon='fluent:compass-northwest-24-regular' width={8} height={8} className='text-blue-400' />
                        <span className='text-gray-400'>
        ...
        <div className='absolute inset-0 bg-black/70 flex items-center justify-center z-40'>
          <div className='bg-gray-800 rounded-lg p-6 max-w-sm w-full mx-4'>
            <div className='flex items-center justify-between mb-4'>
              ...
              >
                <Icon icon='fluent:dismiss-24-regular' width={20} height={20} />
              </button>
            </div>
