...
import IsolatedCameraStream from './IsolatedCameraStream';
...
export default function MidArea_Monitoring({
  ...
  // Function to handle start monitoring
  ...
  // Function to show stop monitoring result
  const showStopResult = (result: StopMonitoringResultType) => {
    setStopResult(result);
  };

  // Function to handle stop monitoring
  ...
  const handleRecordClick = async () => {
    try {
      if (recordingState === 'idle') {
        // Start recording
        const response = await fetch('http://localhost:4000/start-recording', {
          method: 'POST',
        });
        if (response.ok) {
        ...
        // Stop recording
        const response = await fetch('http://localhost:4000/stop-recording', {
          method: 'POST',
        });
        if (response.ok) {
  ...
  return (
    ...
    >      <div className='relative flex-1 w-full h-[300px] md:h-full rounded-2xl overflow-hidden'>
        {/* 🎥 ISOLATED CAMERA STREAM - No Firebase interference */}
        <IsolatedCameraStream metadata={latestData?.metadata} />
      </div>
            <motion.div
              key='stream-on'
              initial={{ opacity: 0 }}
            ...
            >
              ...
              <div className='absolute bottom-4 right-4 flex flex-col gap-2 z-20'>
                {/* Disconnect Button - Only show when stream is loaded */}
                {cameraUrl && isStreamLoaded && (
                  <button
                    onClick={handleStreamDisconnect}
                    className='p-3 rounded-xl transition-all duration-200 backdrop-blur-sm bg-red-500/90 hover:bg-red-600/90 text-white hover:scale-105'
            ...
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.3 }}
              className='flex flex-col gap-2.5 p-4 items-center justify-center bg-gradient-to-br from-blue-500/10 to-blue-400/10 w-full h-full rounded-2xl border-2 border-blue-500/20 relative'
            >
              {' '}
              {/* Configure Button - Top Right Corner */}
              <button
                onClick={openSettingsPage}
                className={`absolute top-4 right-4 p-3 rounded-xl transition-all duration-200 backdrop-blur-sm ${
                  isDark
                    ? 'bg-green-500/20 hover:bg-green-500/30 text-green-400 border border-green-500/30'
                    : 'bg-green-50 hover:bg-green-100 text-green-600 border border-green-200'
                }`}
                title='Go to Settings to Configure Camera'
              >
                <Icon
              ...
              </p>              <div className='flex flex-row items-center gap-2 px-4 py-2 bg-gradient-to-br from-blue-500 to-blue-400 rounded-full text-white text-sm text-center'>
